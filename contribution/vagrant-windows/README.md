...
5. Run the full test suite (including file enforcement tests):

```powershell
$env:KUBEARMOR_DRIVER_AVAILABLE = "true"
cd C:\KubeArmor\tests\nonk8s_env
ginkgo -r --vv --flake-attempts=3 --timeout=20m .\windows_policy\
```

> **Without the driver**, set `KUBEARMOR_DRIVER_AVAILABLE=false` (the
> default in the VM) and only the AppLocker process-enforcement tests run.
