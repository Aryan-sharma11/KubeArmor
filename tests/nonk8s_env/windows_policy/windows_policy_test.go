...
// driverAvailable returns false when KUBEARMOR_DRIVER_AVAILABLE=false is set
// in the environment. Used to gate minifilter-dependent tests.
func driverAvailable() bool {
	if v, ok := os.LookupEnv("KUBEARMOR_DRIVER_AVAILABLE"); ok {
		switch v {
		case "false", "0", "no", "FALSE", "NO":
			return false
		}
	}
	return true // default: assume driver present
}
...
var _ = Describe("Windows Policy Tests", func() {
	...
	// loaded. On GitHub-hosted windows-2022 runners without
	// test-signing mode, set KUBEARMOR_DRIVER_AVAILABLE=false to
	// skip this suite automatically.
	// ─────────────────────────────────────────────────────────
	Describe("File Block Enforcement (Minifilter Driver)", func() {
		BeforeEach(func() {
			if !driverAvailable() {
				Skip("KUBEARMOR_DRIVER_AVAILABLE=false — minifilter not loaded, skipping file enforcement tests")
			}
		})
	...
	Describe("Process Audit Enforcement (Minifilter Driver)", func() {
		BeforeEach(func() {
			if !driverAvailable() {
				Skip("KUBEARMOR_DRIVER_AVAILABLE=false — minifilter not loaded, skipping process audit tests")
			}
		})
	...
	Describe("File Audit Enforcement (Minifilter Driver)", func() {
		BeforeEach(func() {
			if !driverAvailable() {
				Skip("KUBEARMOR_DRIVER_AVAILABLE=false — minifilter not loaded, skipping file audit tests")
			}
		})
